...
func (l *SocketListener) handleClientCreateNewFaction(client *shared.GameClient, body *models.ClientCreateNewFactionBody) {
	...
	// verify session id
	if body.SessionID != *client.SID {
		shared.TeeLog(fmt.Sprintf("handleClientCreateNewFaction: id spoof attempt: %v vs %v", &body.SessionID, &client.SID))
	} else {
		// initialize services
		msgRegistry := models.NewMessageRegistry()
...
func (l *SocketListener) handleClientApplyToFaction(client *shared.GameClient, body *models.ClientApplyToFactionBody) {
	...
	// verify session id
	if body.SessionID != *client.SID {
		shared.TeeLog(fmt.Sprintf("handleClientApplyToFaction: id spoof attempt: %v vs %v", &body.SessionID, &client.SID))
	} else {
		// initialize services
		msgRegistry := models.NewMessageRegistry()
...
func (l *SocketListener) handleClientApproveApplication(client *shared.GameClient, body *models.ClientApproveApplicationBody) {
	...
	// verify session id
	if body.SessionID != *client.SID {
		shared.TeeLog(fmt.Sprintf("handleClientApproveApplication: id spoof attempt: %v vs %v", &body.SessionID, &client.SID))
	} else {
		// initialize services
		msgRegistry := models.NewMessageRegistry()
...
func (l *SocketListener) handleClientRejectApplication(client *shared.GameClient, body *models.ClientRejectApplicationBody) {
	...
	// verify session id
	if body.SessionID != *client.SID {
		shared.TeeLog(fmt.Sprintf("handleClientRejectApplication: id spoof attempt: %v vs %v", &body.SessionID, &client.SID))
	} else {
		// initialize services
		msgRegistry := models.NewMessageRegistry()
...
func (l *SocketListener) handleClientViewMembers(client *shared.GameClient, body *models.ClientViewMembersBody) {
	...
	// verify session id
	if body.SessionID != *client.SID {
		shared.TeeLog(fmt.Sprintf("handleClientViewMembers: id spoof attempt: %v vs %v", &body.SessionID, &client.SID))
	} else {
		// initialize services
		msgRegistry := models.NewMessageRegistry()
...
func (l *SocketListener) handleClientKickMember(client *shared.GameClient, body *models.ClientKickMemberBody) {
	...
	// verify session id
	if body.SessionID != *client.SID {
		shared.TeeLog(fmt.Sprintf("handleClientKickMember: id spoof attempt: %v vs %v", &body.SessionID, &client.SID))
	} else {
		// initialize services
		msgRegistry := models.NewMessageRegistry()
...
func (l *SocketListener) handleClientViewActionReportsPage(client *shared.GameClient, body *models.ClientViewActionReportsPageBody) {
	...
	// verify session id
	if body.SessionID != *client.SID {
		shared.TeeLog(fmt.Sprintf("handleClientViewActionReportsPage: id spoof attempt: %v vs %v", &body.SessionID, &client.SID))
	} else {
		// initialize services
		msgRegistry := models.NewMessageRegistry()
